...
with open(input_file, 'r', newline='') as input_:
    for play, line in enumerate(input_):
        plays.append((line.rstrip().split(' ')))
        line = line.rstrip()
        

print(plays)
rps: dict[str, tuple[str, int]] = {
#What opponent plays | First column of data
    'A': ("Rock", 1),
    'B': ("Paper", 2),
    'C': ("Scissors", 3),
}

rps['X']: tuple[str, int] = rps['A']
rps['Y']: tuple[str, int] = rps['B']
rps['Z']: tuple[str, int] = rps['C']
...
for play, (opponent, you) in enumerate(plays):
    score += results[opponent][you] + rps[you][1]
    print(f"""Play {play}
Opponent plays: {rps[opponent[0]]}, \
You play: {rps[you][0]},\
The result is a {result[results[opponent][you]]},\
for {results[opponent][you]} points. \
Plus {rps[you][1]}\
points for selecting {rps[you][0]}.
New score: {score}
""")
